本篇介绍 C++ 的 std::condition_variable 用法，使用 std::condition_variable 的 wait 会把目前运行的线程 thread 停下來並且等候事件通知，而在另外一個執行緒裡我們可以使用 std::condition_variable 的 notify_one 或 notify_all 去發送通知那些正在等待的事件，這在多线程程序里经常使用到，以下將開始介紹 std::condition_variable 的用法，並展示一些範例，建議閱讀以下文章前需先對建立 std::thread 多執行緒與std::mutex 鎖有一定程度的熟悉。
...
**例1.** 用 notify_one 通知一個正在 wait 的執行緒
下面的例子是先開一個新的线程 worker_thread 然後使用 std::condition_variable 的 wait 事件的通知，
...
**例2.** 用 notify_all 通知全部多個 wait 等待的執行緒
