...
public:
    explicit OpenCommand(shared_ptr<Television> tv) : tv_(tv) {}
    void execute() {
        tv_->open();
    }
private:
...
public:
    explicit CloseCommand(shared_ptr<Television> tv) : tv_(tv) {}
    void execute() {
        tv_->close();
    }
private:
...
class SwitchChannelCommand : public Command {
public:
    explicit SwitchChannelCommand(shared_ptr<Television> tv) : tv_(tv) {}
    void execute() {
        tv_->changeChannel();
    }
private:
...
private:
    shared_ptr<Command> cmd_;
};

int main() {
    shared_ptr<Television> tv = make_shared<Television>();

    shared_ptr<Command> openCommand = make_shared<OpenCommand>(tv);
    shared_ptr<Command> closeCommand = make_shared<CloseCommand>(tv);
    shared_ptr<Command> switchCommand = make_shared<SwitchChannelCommand>(tv);

    shared_ptr<Controller> controller = make_shared<Controller>();

    controller->setCommand(openCommand);
    controller->executeCommand();
    controller->setCommand(switchCommand);
    controller->executeCommand();
