# <font color="deepskyblue">KD-Tree 数据结构</font>

KD-Tree（k-dimensional Tree）是一种**高维空间数据的索引结构**，核心目标是**加速高维数据的近邻搜索（k-近邻搜索）和范围搜索**，广泛应用于激光SLAM、点云处理、计算机视觉等领域（如点云邻域搜索、特征匹配、ICP配准等）。

直接遍历搜索邻域的时间复杂度为 $O(n^2)$，而KD-Tree可将搜索复杂度降至 $O(n\log n)$，是实现实时SLAM的关键。

## <font color="skyblue">一、KD-Tree的核心概念</font>

### <font color="YellowGreen">1. 什么是KD-Tree？</font>
...
### <font color="YellowGreen">2. 关键术语</font>
...
## <font color="SkyBlue">二、KD-Tree的构建过程（3D点云场景）</font>

KD-Tree的构建是**递归分割空间**的过程，以3D点云（k=3）为例，步骤如下：

### <font color="YellowGreen">1. 构建步骤（递归）</font>
