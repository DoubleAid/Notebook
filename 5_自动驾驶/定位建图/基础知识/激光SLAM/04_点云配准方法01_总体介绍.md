...
1. 预处理：将参考点云划分网格并建模为正态分布
    - 网格划分

        将参考点云（目标点云，记为T）所在的空间均匀划分为若干网格（voxel），例如 3D 空间中每个网格是一个小立方体。网格大小根据点云密度设置（如 0.5m×0.5m×0.5m），太大会丢失细节，太小会导致部分网格点太少无法建模。
  
    - 网格内正态分布建模

        对每个包含足够数量点（如≥5 个）的网格，计算内部所有点的均值（μ） 和协方差矩阵（Σ），将该网格的点云分布建模为一个多元正态分布 p ~ N(μ, Σ)

        这个分布的物理意义是：网格内某位置出现点的概率，越靠近均值（网格中心趋势）概率越高，偏离越远概率越低。

    - 过滤无效网格

        对点太少的网格（如 < 5 个点），因无法可靠估计协方差，标记为无效（概率为 0）。

2. 初始化变换参数

    设源点云（待配准点云，记为S）到参考点云T的变换矩阵为$T(\xi)$，其中$\xi$是待优化的参数（3D 中为 6 个参数：3 个平移 + 3 个旋转，通常用欧拉角或四元数表示）。
  
    初始值可设为单位变换（假设初始姿态已知），或根据先验信息（如里程计）设置一个粗略值。

3. 计算目标函数：源点云在参考分布场中的似然概率

    - **变换源点**：
        
        用当前变换参数$\xi$对源点云S中的每个点$s_i$进行变换，得到$s_i' = T(\xi) \cdot s_i$。
    - **计算每个变换点的概率**:
        
        对每个$s_i'$，找到它所属的参考点云网格，根据该网格的正态分布模型计算概率$p(s_i')$（若落在无效网格，概率为 0）

    - **构建目标函数**

        目标是最大化所有变换点的概率乘积（即似然函数），为便于计算，通常取负对数转化为最小化问题：$\text{cost}(\xi) = -\sum_{i=1}^n \log(p(s_i'))$

        其中n是源点云的点数。这个成本函数的物理意义是：让源点云经过变换后，尽可能落在参考点云概率高的区域（即与参考点云重合度高）。

4. 优化目标函数：求解最优变换参数

    - 梯度下降 / 牛顿法优化：

        通过计算成本函数$\text{cost}(\xi)$对变换参数$\xi$的梯度（一阶导数）和 Hessian 矩阵（二阶导数），采用迭代优化方法（如高斯 - 牛顿法）不断更新$\xi$，直到成本函数收敛（变化量小于阈值）或达到最大迭代次数。
