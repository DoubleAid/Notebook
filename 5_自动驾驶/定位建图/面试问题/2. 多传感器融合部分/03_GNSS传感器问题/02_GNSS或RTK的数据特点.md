...
+ GNSS（如GPS、北斗）（全球导航卫星系统）：提供绝对位置，精度在米级
+ RTK（实时动态差分定位）：提供厘米级精度的绝对位置，能精确到具体的某个点。
...
+ 绝对精度：米级绝对定位，最核心的特点：提供具有绝对地理参考的坐标。
...
+ 全局定位​：
  + GNSS提供米级的全局位置，防止SLAM绝对位置的长期漂移。可以将局部地图对齐到全球坐标系。
  + RTK提供厘米级的全局位置，几乎完全消除SLAM的全局漂移。可以实现真正的全局一致地图。
+ 初始化/重定位​
  + GNSS:提供一个粗略的初始位置，帮助系统快速启动或从跟踪失败中恢复。
  + RTK:提供极其精确的初始位姿，大大减小后续优化的不确定性。
+ 闭环检测​
  + GNSS:提供位置提示，例如“两个地方经纬度接近，可能是同一个地方”，辅助视觉/激光闭环。
  + RTK:直接提供强有力的绝对位置约束，本身就可以作为最强的闭环检测，无需识别特征。
+ 多传感器融合​
  + GNSS:与IMU、轮速计等融合，约束惯导的漂移。通常作为松耦合的观测项。
  + RTK:可以作为真值传感器，用于在线标定其他传感器（如相机-IMU外参、轮子里程计参数）的尺度。
