...
+ 角速率零偏$\mathbf{b}_{\omega0}$：直接取 N 帧角速度数据的均值（静止时真实角速率为 0，均值即零偏）；
+ 初始姿态（重力方向）：在静止段内，取加速度计测量值的均值 $a_{avg}$。计算一个旋转矩阵 R，使得 $R⋅a_{avg}$ 与 $[0,0,−g]^⊤$方向对齐。此 R即为初始姿态矩阵，确定了俯仰和横滚角。
+ 加速度计零偏 $b_a$（粗略）：将 $a_{avg}$ 用 R变换到世界系，与世界系重力矢量 $[0,0,−g]^⊤$比较，将差值反变换回本体坐标系，作为 $b_a$ 的初始估计。或简单地将 $b_a$ 初始化为0。
