...
1. 位姿参数化
    ...
    将位姿 $\mathbf{T}$ 用李代数 $\boldsymbol{\xi}$ 表示（6维向量：3维旋转+3维平移），避免直接优化旋转矩阵的正交性约束。

2. **计算代价函数的梯度与海森矩阵**
   - 梯度 $\nabla \mathcal{F}$：代价函数对李代数 $ \boldsymbol{\xi}$ 的一阶导数，反映代价函数的下降方向；
   - 海森矩阵 $\mathbf{H}$：代价函数对 $\boldsymbol{\xi}$ 的二阶导数，近似为 $\mathbf{J}^T \mathbf{J}$（$\mathbf{J}$ 是残差的雅可比矩阵）。
3. **求解位姿增量**
   通过解线性方程组得到最优位姿增量 \( \Delta \boldsymbol{\xi} \)：
   $$
   \mathbf{H} \Delta \boldsymbol{\xi} = -\nabla \mathcal{F}
   $$
4. **更新位姿并迭代**
   - 将增量 \( \Delta \boldsymbol{\xi} \) 通过**指数映射**转换为新的位姿 \( \mathbf{T}_{\text{new}} = \exp(\Delta \boldsymbol{\xi}^\times) \cdot \mathbf{T}_{\text{old}} \)；
   - 重复步骤二~四，直到满足收敛条件。

## 五、 收敛判断与结果输出
迭代停止的条件通常有3个，满足任意一个即可结束配准：
1. **位姿增量阈值**：相邻两次迭代的位姿增量 \( \|\Delta \boldsymbol{\xi}\| < \tau_1 \)（工程中 \( \tau_1=1e-4 \sim 1e-3 \, \text{rad/m} \)）；
2. **代价函数变化阈值**：代价函数的变化量 \( |\mathcal{F}_{\text{new}} - \mathcal{F}_{\text{old}}| < \tau_2 \)（\( \tau_2=1e-6 \)）；
3. **最大迭代次数**：达到预设的最大迭代次数（如50~100次），防止算法陷入死循环。
